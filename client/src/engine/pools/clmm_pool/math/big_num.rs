...
macro_rules! construct_bignum {
    ( $(#[$attr:meta])* $visibility:vis struct $name:ident ( $n_words:tt ); ) => {
        ...
        impl core_::convert::From<u128> for $name {
        ...
        impl core_::convert::TryFrom<$name> for u128 {
            type Error = &'static str;

            #[inline]
            fn try_from(u: $name) -> core_::result::Result<u128, &'static str> {
                let $name(arr) = u;
                for i in 2..$n_words {
                    if arr[i] != 0 {
        ...
        impl core_::convert::TryFrom<$name> for i128 {
            type Error = &'static str;

            #[inline]
            fn try_from(u: $name) -> core_::result::Result<i128, &'static str> {
    ...
    ( @construct $(#[$attr:meta])* $visibility:vis struct $name:ident ( $n_words:tt ); ) => {
        ...
        impl core_::default::Default for $name {
            fn default() -> Self {
                $name::zero()
            }
        }

        impl core_::ops::BitAnd<$name> for $name {
            type Output = $name;
        ...
        impl core_::ops::BitOr<$name> for $name {
            type Output = $name;
        ...
        impl core_::ops::BitXor<$name> for $name {
            type Output = $name;
        ...
        impl core_::ops::Shl<usize> for $name {
            type Output = $name;
        ...
        impl<'a> core_::ops::Shl<usize> for &'a $name {
            type Output = $name;
            fn shl(self, shift: usize) -> $name {
                *self << shift
            }
        ...
        impl core_::ops::Shr<usize> for $name {
            type Output = $name;
        ...
        impl<'a> core_::ops::Shr<usize> for &'a $name {
            type Output = $name;
            fn shr(self, shift: usize) -> $name {
                *self >> shift
            }
