use std::{collections::{HashMap}, fs::{read_dir, read_to_string}, str::FromStr};
...
impl Graph {
    ...
    fn insert_edge(&mut self, pool: PoolInfo, node0_index: usize, node1_index: usize) -> Result<usize> {
...
mod tests {
    ...
    fn test_insert_node_with_invalid_address_returns_error() {
        ...
        assert!(result.is_err(), "Expected insert_node to return an error for invalid address");
    }
    ...
    fn test_insert_edge_add_one_edge_returns_index() {
        ...
        let idx1 = graph.insert_node(TokenInfo { 
            address: Some("Czfq3xZZDmsdGdUyrNLtRhGc47cXcZtLG4crryfu44zE".to_string()), 
            decimals: Some(18), 
            name: Some("Test Name".to_string()), 
            symbol: Some("Test Symbol".to_string()) 
        ...
        let idx2 = graph.insert_node(TokenInfo { 
            address: Some("7eMnzvi48Nbz2yRaQrCWqfQ7awPNPfV3AboaejktyGMD".to_string()), 
            decimals: Some(18), 
            name: Some("Test Name".to_string()), 
            symbol: Some("Test Symbol".to_string()) 
